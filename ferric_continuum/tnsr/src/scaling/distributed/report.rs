...
//! all-reduce, pipeline handoff), and uses [`super::super::roofline`] to flag
//! whether the whole step is comm-bound or compute-bound on the given hardware.
...
pub fn distributed_report(
    ...
    // TP all-reduce (per layer × num_layers for the TP term) + logical PP
...
fn symbolic_collectives(
    ...
    if dp > 1 {
        collectives.push(SymbolicCollective {
            ...
            participants: dp,
            reason: "replicated parameters need synchronized gradients",
        });
    }
