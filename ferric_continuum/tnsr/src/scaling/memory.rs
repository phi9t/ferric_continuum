...
//!   level: save attention scores, attention mix, and small softmax sites;
//!   recompute GELU, LayerNorm, and large Linear sites.
...
pub enum ActivationCheckpointing {
    ...
    /// AttentionScores and AttentionMix, save only small Softmax sites, and
    /// recompute GELU, LayerNorm, and large Linear sites.
    Selective,
...
/// keeps the attention sites saved by `TransformerSelectivePolicy`: one
/// `AttentionScores` site (`B*T*T`), one `AttentionMix` site (`B*T*D`), and the
/// `Softmax` site (`B*T*T`) only when its byte size is below the small-save
/// threshold. Full checkpointing keeps no internal sites.
...
impl ActivationSites {
    ...
    fn selective_elements(self, elem_bytes: u64) -> u64 {
        let attention_scores = self.attention_square_elements;
        let attention_mix = self.token_dim_elements;
        let softmax = kept_small_site_elements(self.attention_square_elements, elem_bytes);
        attention_scores + attention_mix + softmax
    }
}
