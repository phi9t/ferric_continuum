...
fn test_distributed_report_summarizes_hybrid_parallelism() {
    ...
    assert_eq!(r.symbolic_collectives[2].participants, 2);
    assert_eq!(r.symbolic_collectives[3].participants, 3);
    assert_eq!(r.symbolic_collectives[4].participants, 4);
...
#[test]
fn test_distributed_report_uses_ddp_gradient_sync_for_stage1_only() {
    let mesh = DeviceMesh::new_2d(2, "dp", 2, "tp");
    let r = distributed_report(&tiny(), 4, &mesh, ZeroStage::Stage1, &a100_bf16());
    ...
    let names: Vec<&str> = r
        ...
        .map(|collective| collective.name)
        .collect();
    assert!(names.contains(&"DDP gradient all-reduce"));
    assert!(!names.contains(&"FSDP gradient reduce-scatter"));
    assert!(!names.contains(&"FSDP parameter all-gather"));
}
